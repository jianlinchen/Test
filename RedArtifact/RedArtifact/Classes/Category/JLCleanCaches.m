...
#import "JLCleanCaches.h"

@implementation JLCleanCaches
...
+ (BOOL)clearCachesFromDirectoryPath:(NSString *)dirPath{
    ...
    //获得全部文件数组
    NSArray *fileAry =  [JLCleanCaches getAllFileNames:dirPath];
    //遍历数组
    BOOL flag = NO;
    for (NSString *fileName in fileAry) {
        NSString *filePath = [dirPath stringByAppendingPathComponent:fileName];
        flag = [JLCleanCaches clearCachesWithFilePath:filePath];
