...
{
//    (70.0/320.0)*SCREEN_WIDTH
    
    double p= (double) point_x ;
    double pp=(p/320.0)*SCREEN_WIDTH;
    UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(pp, 50, 20, 20)];
...
- (NSInteger)numberOfComponentsInPickerView:(UIPickerView *)pickerView
{
    if (self.datePickerStyle == UUDateStyle_YearMonthDayHourMinute){
        if (isIOS7) {
            [self creatValuePointXs:@[@"90",@"150",@"215",@"280",@"300"]
                          withNames:@[@"年",@"月",@"日",@"时",@""]];
        }
        return 5;
    }
...
- (CGFloat)pickerView:(UIPickerView *)pickerView widthForComponent:(NSInteger)component
{
    switch (self.datePickerStyle) {
        case UUDateStyle_YearMonthDayHourMinute:{
            ...
            if (component==0) return (80.0/320.0)*SCREEN_WIDTH;
            if (component==1) return (60.0/320.0)*SCREEN_WIDTH;
            if (component==2) return (60.0/320.0)*SCREEN_WIDTH;
            if (component==3) return (60.0/320.0)*SCREEN_WIDTH;
            if (component==4) return (00.0/320.0)*SCREEN_WIDTH;
        }
            break;
