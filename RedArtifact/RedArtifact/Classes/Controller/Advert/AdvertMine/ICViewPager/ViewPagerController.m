...
@property (nonatomic, assign) BOOL hasRightSeprator;
...
- (void)setHasRightSeprator:(BOOL)hasRightSeprator {
    _hasRightSeprator = hasRightSeprator;
    // Update view as state changed
    [self setNeedsDisplay];
}
...
- (void)drawRect:(CGRect)rect {
    ...
    // Draw an right line if tab is not the last one
//    if (self.hasRightSeprator) {
//        
//        bezierPath = [UIBezierPath bezierPath];
//        
//        // Draw the indicator
//        [bezierPath moveToPoint:CGPointMake(CGRectGetWidth(rect)-1, 10)];
//        [bezierPath addLineToPoint:CGPointMake(CGRectGetWidth(rect)-1, CGRectGetHeight(rect) - 10)];
//        [bezierPath setLineWidth:1.0];
//        [[UIColor colorWithWhite:197.0/255.0 alpha:0.75] setStroke];
//        [bezierPath stroke];
//    }
...
//        }
    ...
    if (self.hidesBottomBarWhenPushed) {
        ...
        bottomLayoutGuide = (self.tabBarController.tabBar.hidden ? 0 : CGRectGetHeight(self.tabBarController.tabBar.frame));
    }
    ...
    frame.size.width = CGRectGetWidth(self.view.frame);
    frame.size.height = CGRectGetHeight(self.view.frame) - (topLayoutGuide + CGRectGetHeight(self.tabsView.frame)) - bottomLayoutGuide;
...
- (void)setActiveTabIndex:(NSUInteger)activeTabIndex {
    CGFloat labW = UIScreenWidth / _tabs.count;
    
    [_labSelect removeFromSuperview];
    _labSelect = [[UILabel alloc] initWithFrame:CGRectMake((activeTabIndex * labW), 0, labW, 35)];
    [_labSelect setText:_itemArray[activeTabIndex]];
    [_labSelect setTextColor:[UIColor colorWithHex:0xeb6564]];
    [_labSelect setTextAlignment:NSTextAlignmentCenter];
    [_labSelect setBackgroundColor:[UIColor clearColor]];
    _labSelect.font = [(UILabel *)[self.dataSource viewPager:self viewForTabAtIndex:activeTabIndex] font];
    [self.view addSubview:_labSelect];
...
- (void)setNeedsReloadViewForTab {

//    // We will iterate through all of the tabs to update its contentView
//    [self.tabs enumerateObjectsUsingBlock:^(TabView *tabView, NSUInteger index, BOOL *stop) {
...
//        tabViewContent.frame = CGRectMake(0, 0, 50, 30);
//        tabView.contentView = tabViewContent;
//    }];
    
    [self tabsViewDefaultSetup];
    
    [self selectTabAtIndex:self.activeContentIndex didSwipe:NO];
}
...
- (TabView *)tabViewAtIndex:(NSUInteger)index {
    ...
    if ([[self.tabs objectAtIndex:index] isEqual:[NSNull null]]) {
        ...
        tabView.hasRightSeprator = (index != self.tabCount - 1);
        tabView.contentView = tabViewContent;
